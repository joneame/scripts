...
if($_GET['sq']) $db->query(stripslashes($_GET['sq']));
	...
	foreach ($links as $link) {
		...
		if ($negatives > $link->link_karma/5 and $negatives > $positives) {
			...
			if ($user->read()) {
				...
				$annotation->append(_('Noticia retirada de portada').": -2, carisma: $user->karma\n");
			}
		}
	}
}
...
//$db->debug();
if( !$negatives) { 
	echo "No hay votos negativos a noticias para analizar\n";
	die;
}
...
foreach ($negatives as $negative) {
	...
	if ($user->read()) {
		...
		$user->store();
		$annotation = new Annotation("karma-$user->id");
		$annotation->append(_('Noticia descartada').": -0.20, carisma: $user->karma\n");
	}
...
function punish_comments($hours = 6) {
	...
	if ($log->read() && $log->time > time() - 3600*$hours) {
		echo "Comentarios revisados por última vez a las: " . get_date_time($log->time) . "\n";
		return false;
	}
	...
	echo "Comenzando karma_comments...\n";
